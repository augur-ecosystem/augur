...
def add_staff(staff_properties):
    ...
    """
    staff = db.Staff(**staff_properties)
    orm.commit()
    return staff
...
def add_team(team_properties):
    ...
    """
    team = db.Team(**team_properties)
    orm.commit()
    return team


def add_staff_to_team(team_or_id, staff):
    """
    Adds one or more staff members to the team.
    :param team_or_id: The ID of the team to add the staff to
    :param staff: A list of dictionary objects or a list of Staff objects
    :return: The team object
    """
    if not isinstance(staff, (list,tuple)):
        staff = [staff]

    if not isinstance(team_or_id, db.Team):
        team = db.Team[team_or_id]
    else:
        team = team_or_id
    ...
    for s in staff:
        ...
        if isinstance(s, dict):
            schema = StaffSchema()
            result = schema.load(s)
            if result:
                staff_ob = result.data

        elif isinstance(s, db.Staff):
            staff_ob = s

        if staff_ob:
            team.members.add(staff_ob)
        else:
            raise TypeError("Unknown type given for one of the staff members")
...
def log_event_data(event_type, event_data):
    ...
    el = EventLog(event_time=event_time,
